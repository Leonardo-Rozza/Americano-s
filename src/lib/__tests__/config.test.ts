...
describe("config", () => {
  ...
  it("usa defaults permisivos cuando faltan variables", async () => {
    const { appConfig, getJwtSecret, getLoginRateLimitSecret } = await import("../config");
    ...
    expect(getLoginRateLimitSecret()).toBe("dev-login-rate-limit-secret");
  });
  ...
  it("normaliza y parsea configuracion desde variables de entorno", async () => {
    ...
    vi.stubEnv("LOGIN_RATE_LIMIT_SECRET", " rate-limit-secret ");
    ...
    expect(getLoginRateLimitSecret()).toBe("rate-limit-secret");
  });
});
